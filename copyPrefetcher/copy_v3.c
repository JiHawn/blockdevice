...
void* copy(void* p) {
    DIR *srcDir, *destDir;
    int srcFd[numOfFile/numOfPair+1];
    int destFd[numOfFile/numOfPair+1];
    ...
    while((ent = readdir(srcDir)) != NULL) {
        ...
        if((srcFd[i] = open(srcFilepath, O_RDONLY)) < 0) {
            perror("src file open");
            exit(1);
        }
        if((destFd[i] = open(destFilepath, O_RDWR | O_CREAT| O_TRUNC, 0666)) < 0) {
    ...
    for(int i=0; i<numOfFile/numOfPair+1; i++) {
        if(srcFd[i] < 0) continue;
        while(copyLock[num]) {;}
        if((res = read(srcFd[i], buffer, size[i])) < 0) {
            perror("src file read");
            exit(1);
        }
        if((res = write(destFd[i], buffer, size[i])) < 0) {
